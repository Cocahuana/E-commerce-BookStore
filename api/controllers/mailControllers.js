...
const specificOffer = async (req, res, next) =>{
    ...
    try{
        ...
        if(!bookObject.salePrice) return res.send(`${bookObject.title} does not have a sale currently`)
        ...
        let usersToMail = usersToFilter.filter(user => user.favorites.includes(bookId))

        if(usersToMail.length === 0) return res.send("No users with that book in their favorites")
...
module.exports = { orderConfirmation, userCreated, newsletterUpdate, newOffers, passwordRecovery, specificOffer }